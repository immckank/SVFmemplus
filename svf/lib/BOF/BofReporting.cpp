                                                      ...
                                                      const SVFVar* indexVar)
{
    ...
    // Filter the detected access before it becomes a PendingReport. This is the
    // common analysis-time detection boundary for GEP and memory-operation
    // checks, not a post-processing pass over emitted warnings.
    if (!loc)
        return;
    if (const SaberSemanticRules::Fact* fact =
            SaberSemanticRules::get()->ignoredSource("bof", loc->getSourceLoc()))
    {
        SaberSemanticRules::get()->recordHit(
            *fact, "bof", "access", loc->getSourceLoc());
        return;
    }
